...
def generate_examples(examples):
    example_template = 'The topic of the verse "{example_text}" is {example_category}'
    example_strs = [
...
def format_text_with_examples(example, few_shot_examples):
    template = """Topic Classification: Recommendation, Faith, Description, Sin, Grace, Violence.

{examples}
The label of "{text}" is
"""
...
def get_ans(text, tokenizer, model, device, category_tokens):
    ...
    options_list = [
        (logits[category_tokens["Recommendation"]], "Recommendation"),
        (logits[category_tokens["Faith"]], "Faith"),
        (logits[category_tokens["Description"]], "Description"),
        (logits[category_tokens["Sin"]], "Sin"),
        (logits[category_tokens["Grace"]], "Grace"),
        (logits[category_tokens["Violence"]], "Violence"),
    ]
...
def load_tsv_data(file_path):
    col_names = ["index_id", "category", "text"]
    return pd.read_csv(
        file_path,
        delimiter="\t",
        names=col_names,
...
def main():
    ...
    category_tokens = {
        "Recommendation": tokenizer(" Recommendation").input_ids[-1],
        "Faith": tokenizer(" Faith").input_ids[-1],
        "Description": tokenizer(" Description").input_ids[-1],
        "Sin": tokenizer(" Sin").input_ids[-1],
        "Grace": tokenizer(" Grace").input_ids[-1],
        "Violence": tokenizer(" Violence").input_ids[-1],
    }
    ...
    for lang in args.configs:
        ...
        if os.path.exists(output_path):
            print(f"Skip {lang}")
            continue
        train_path = f"{base_path}/{lang}/{lang}_train.tsv"
        test_path = f"{base_path}/{lang}/{lang}_test.tsv"
    ...
    print(
        f"All correct predictions: {all_correct_predictions} All predictions: {all_total_predictions}"
    )
    print(f"Accuracy for all language: {all_accuracy:.4f}")

    # write2sheet("Taxi-1500", result_df)
